# Phase 5i: Integration Check

Validate the generated integration application code.

## Phase Objective

Perform validation checks on the generated integration to ensure it's correctly structured, dependencies resolve, imports work, and configuration exports successfully.
...
- `application.py` - Application class with `on_ingestion_endpoint`
...
Read and verify `doover_config.json` has correct integration configuration:
...
### Step 7: Validate Ingestion Handler

Check that the application has the required ingestion endpoint handler:

```bash
cd {app-directory}
uv run python -c "
from {app_name}.application import Application
app = Application()
assert hasattr(app, 'on_ingestion_endpoint'), 'Missing on_ingestion_endpoint method'
print('Ingestion handler OK')
"
```

**Expected**: Prints "Ingestion handler OK" without errors.

**If errors occur**:
- Note which method is missing
- Report in summary

### Step 8: Update State

Update `.appgen/PHASE.md`:
- Set current phase to "Phase 5 - Integration Check"
...
| Ingestion Handler | PASS/FAIL | {issues if any} |
